import doFetch0 from "../api/fetch"
...
async function getPartWithDelay(delay: number) {
    ...
    const { size, content: rawContent } = await doFetch0("http://localhost:8088/get-part")
    const content =
...
//         const data = await doFetch0(`http://localhost:8088/get-branch?path=${path}`)
